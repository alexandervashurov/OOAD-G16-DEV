...
public class AdminLoginView extends HorizontalLayout implements View, Button.ClickListener, Serializable {
	...
	public AdminLoginView() {
		...
		recovButton = new Button("Forgot password?", this);
		recovButton.setWidth("200px");
		recovButton.addStyleName(ValoTheme.BUTTON_PRIMARY);
		recovButton.setImmediate(true);
		recovButton.addClickListener((e)->recovButtonClick(e));
	...
	//	adminMessagesTable.setVisibleColumns("inputEvent", "message");
		...
		FormLayout FL = new FormLayout(username, password, loginButton, recovButton);
		...
		setMargin(true);
		setExpandRatio(leftVL, 20);
		setExpandRatio(rightVL, 4);
	}
	
	@Override
	public void enter(ViewChangeEvent event) {
		log.debug("Enter method invokation");
		username.setValue("");
	...
	public void recovButtonClick(ClickEvent event){
		CtAdministrator ctAdmin =  (CtAdministrator) sys.getCtAuthenticated(actAdmin);
		ctAdmin.vpNeedRecovery = new PtBoolean(true);
		Page.getCurrent().reload();
	}
}
